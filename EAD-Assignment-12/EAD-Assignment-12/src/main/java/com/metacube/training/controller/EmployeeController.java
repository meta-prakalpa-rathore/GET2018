...
public class EmployeeController {
	...
	public ModelAndView login(@RequestParam("username") String username, 
		...
		if(employeeService.isValidLogin(username, password))
			view = "redirect:/employee/dashboard?email="+username;
		else
			view = "employee/login";
	...
	public ModelAndView searchEmployee(@RequestParam("criteria") String criteria1, @RequestParam("keyword") String keyword){
	    ...
	    if("name".equals(criteria1))
	    	criteria = SearchCriteria.NAME;
	    else if("project".equals(criteria1))
	    	criteria = SearchCriteria.PROJECT;
	    else if("skill".equals(criteria1))
	    	criteria = SearchCriteria.SKILL;
	    else if("experience".equals(criteria1))
	    	criteria = SearchCriteria.EXPERIENCE;
	    else
	    	criteria = null;
	    
		List<Employee> searchResult = employeeService.searchEnabledEmployee(criteria, keyword);
